...
firewall-cmd --permanent --policy NetToFW --add-service=dns
  \end{verbatim}
%   Además, desde el firewall externo creamos unas nuevas reglas en el fichero \emph{/etc/shorewall/rules}
%   para permitir el tráfico DNS:
%   \begin{verbatim}
...
%   \end{verbatim}
  ...
  % Para ello, se va a configurar un port forwarding a través de añadir reglas de DNAT en \emph{/etc/shorewall/rules}:
%   \begin{verbatim}
% #ACTION         SOURCE          DEST                     PROTO   DPORT
